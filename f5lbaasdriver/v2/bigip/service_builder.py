...
class LBaaSv2ServiceBuilder(object):
    ...
    def build(self, context, loadbalancer,
              agent, target=None, resource=None,
              agent_hosts=None):
        """Get full service definition from loadbalancer ID."""
        ...
        with context.session.begin(subtransactions=True):
            ...
            if self.resource == constants_v2.LISTENER:
                ...
                service['listeners'] = self._get_listeners(context,
                                                           loadbalancer)
            elif self.resource in (constants_v2.L7POLICY, constants_v2.POOL):
    ...
    def _populate_member_network(self, context, member, network, agent_hosts=None):
        ...
        filters = {'device_owner': ['network:f5lbaasv2'], 'network_id': [network['id']]}
        port = self.plugin.db._core_plugin.get_ports(context, filters)
        LOG.debug('XXXX filter hbp port:%s' % port)
        if port:
            port_id = port[0]['id']
            segment_data = self.disconnected_service.get_segment_id(
